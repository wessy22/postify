...
// פונקציה לבדיקה אם צריך לפרסם עכשיו לפי זמן הפרסום
function shouldPublishNow(publishTime, currentTime = new Date(), toleranceMinutes = 30) {
  if (!publishTime) {
    console.log(`📋 אין זמן פרסום מוגדר - ממשיך בפרסום`);
    return true; // אם אין זמן מוגדר, פרסם בכל מקרה
  }
  
  const currentTimeStr = currentTime.toLocaleTimeString('he-IL', { 
  ...
  console.log(`🕒 זמן נוכחי: ${currentTimeStr}, זמן פוסט: ${publishTime}`);
  
  // המרה לדקות מתחילת היום
  ...
  // חישוב מרחק (כמה דקות הפרש)
  const distance = Math.abs(publishMinutes - currentMinutes);
  
  console.log(`📏 מרחק זמן: ${distance} דקות (טווח מותר: ±${toleranceMinutes} דקות)`);
  
  const shouldPublish = distance <= toleranceMinutes;
  console.log(`⏰ האם לפרסם עכשיו? ${shouldPublish ? 'כן' : 'לא'}`);
...
function selectPostsForDay(allPosts, today = new Date()) {
  ...
  const scheduledPosts = allPosts.filter(post => {
    ...
    if (isScheduled) {
      console.log(`✅ פוסט מתוזמן נמצא: ${post.filename} (${post.schedule_type}) - זמן: ${post.publish_time || 'לא מוגדר'}`);
    }
    return isScheduled;
  });
  ...
  // לוגיקה משופרת: פוסט אחד בלבד ביום עם עדיפות למתוזמנים ובחירה לפי זמן
  console.log(`📋 מדיניות: פרסום פוסט אחד בלבד ביום עם עדיפות למתוזמנים`);
  
  if (validScheduledPosts.length > 0) {
    // יש פוסטים מתוזמנים - בחר את הקרוב ביותר לזמן הנוכחי
    
    // מיון לפי מרחק זמן מהזמן הנוכחי
    const postsWithTimeDistance = validScheduledPosts.map(post => ({
      ...post,
      timeDistance: calculateTimeDistance(post.publish_time, today)
    }));
    
    // מיון לפי מרחק זמן (הקרוב ביותר ראשון)
    postsWithTimeDistance.sort((a, b) => a.timeDistance - b.timeDistance);
    
    const selectedPost = postsWithTimeDistance[0];
    selectedPosts.push(selectedPost);
    
    console.log(`⏰ נבחר פוסט מתוזמן לפי זמן: ${selectedPost.filename} (זמן: ${selectedPost.publish_time || 'לא מוגדר'}, מרחק: ${selectedPost.timeDistance} דקות)`);
    ...
    if (validScheduledPosts.length > 1) {
      ...
      // הצגת רשימת הפוסטים שלא נבחרו
      const notSelectedPosts = postsWithTimeDistance.slice(1);
      notSelectedPosts.forEach(post => {
        console.log(`   ⏭️ לא נבחר: ${post.filename} (זמן: ${post.publish_time || 'לא מוגדר'}, מרחק: ${post.timeDistance} דקות)`);
      });
    }
  } else if (activePosts.length > 0) {
    // אין פוסט מתוזמן - בחר פוסט פעיל לפי זמן פרסום או רוטציה
    console.log(`📋 אין פוסטים מתוזמנים להיום - בודק פוסטים פעילים לפי זמן פרסום`);
    
    // בדוק אם יש פוסטים פעילים עם publish_time
    const activePostsWithTime = activePosts.filter(post => post.publish_time);
    
    if (activePostsWithTime.length > 0) {
      // יש פוסטים פעילים עם זמן - בחר לפי הזמן הקרוב ביותר
      const postsWithTimeDistance = activePostsWithTime.map(post => ({
        ...post,
        timeDistance: calculateTimeDistance(post.publish_time, today)
      }));
      ...
      console.log(`⏰ נבחר פוסט פעיל לפי זמן: ${selectedPost.filename} (זמן: ${selectedPost.publish_time}, מרחק: ${selectedPost.timeDistance} דקות)`);
      
      if (activePostsWithTime.length > 1) {
        console.log(`📋 מתעלם מ-${activePostsWithTime.length - 1} פוסטים פעילים נוספים עם זמן (פוסט אחד בלבד ביום)`);
        
        // הצגת רשימת הפוסטים שלא נבחרו
        const notSelectedPosts = postsWithTimeDistance.slice(1);
        notSelectedPosts.forEach(post => {
          console.log(`   ⏭️ לא נבחר: ${post.filename} (זמן: ${post.publish_time}, מרחק: ${post.timeDistance} דקות)`);
        });
      }
    } else {
      // אין פוסטים פעילים עם זמן - השתמש ברוטציה הרגילה
      console.log(`📋 פוסטים פעילים ללא זמן פרסום - משתמש ברוטציה רגילה`);
...
// פונקציה משופרת לבחירת פוסטים עם מניעת כפילויות ותמיכה ב-publish_time
function selectPostsForDayFixed(allPosts, today = new Date()) {
  const todayStr = today.toISOString().slice(0, 10);
  ...
  // לוגיקה רגילה: מתוזמנים קודם, אחר כך פעילים ברוטציה
  console.log(`📋 מדיניות: פרסום פוסט אחד בלבד ביום עם עדיפות למתוזמנים`);
  
  if (validScheduledPosts.length > 0) {
    // יש פוסטים מתוזמנים - בחר את הראשון (לפי הסדר הרגיל)
    const selectedPost = validScheduledPosts[0];
    selectedPosts.push(selectedPost);
    
    console.log(`⏰ נבחר פוסט מתוזמן: ${selectedPost.filename} (זמן: ${selectedPost.publish_time || 'לא מוגדר'})`);
    
    if (validScheduledPosts.length > 1) {
      console.log(`📋 מתעלם מ-${validScheduledPosts.length - 1} פוסטים מתוזמנים נוספים (פוסט אחד בלבד ביום)`);
    }
  } else if (activePosts.length > 0) {
    // אין פוסט מתוזמן - בחר פוסט פעיל לפי רוטציה רגילה
    console.log(`📋 אין פוסטים מתוזמנים להיום - עובר לפוסטים פעילים`);
    ...
    selectedPosts.push(selectedPost);
    console.log(`🔄 נבחר פוסט פעיל (רוטציה): ${selectedPost.filename} (זמן: ${selectedPost.publish_time || 'לא מוגדר'})`);
  } else {
    // אין פוסטים מתוזמנים ואין פוסטים פעילים
    console.log(`📋 אין פוסטים זמינים להיום - לא מתוזמנים ולא פעילים`);
  }
  ...
  // ===== שלב שני: בדיקת זמן פרסום =====
  if (selectedPosts.length > 0) {
    const selectedPost = selectedPosts[0];
    console.log(`\n🕒 ===== בדיקת זמן פרסום =====`);
    
    const shouldPublish = shouldPublishNow(selectedPost.publish_time);
    
    if (!shouldPublish && selectedPost.publish_time) {
      console.log(`⏰ זמן הפרסום ${selectedPost.publish_time} לא בטווח המותר - כיבוי השרת`);
      return []; // מחזיר מערך ריק כדי לכבות את השרת
    } else {
      console.log(`✅ אושר לפרסום: ${selectedPost.filename}`);
    }
  }
...
(async () => {
  try {
    ...
    const forceRun = args.includes("--force"); // לדלג על בדיקת שבת לצורך בדיקה
    const dryRun = args.includes("--dry-run"); // בדיקת לוגיקה בלבד ללא פרסום
    ...
    // בדיקה אם היום חג או יום זיכרון (הוסר: בדיקת שבת)
    if (!forceRun && jewishHolidaysAndMemorials.includes(todayStr)) {
    ...
    if (specificPostFile) {
      ...
      postsToday = selectPostsForDayFixed(validatedPosts, today);
    }
    ...
    // --- הפעלת הלולאה החדשה (או הצגת מידע בלבד אם dry-run) ---
    if (dryRun) {
      log("🔍 מצב DRY-RUN: בדיקת לוגיקה בלבד ללא פרסום");
      log("📅 זמן נוכחי:", new Date().toLocaleString('he-IL', { timeZone: 'Asia/Jerusalem' }));
      
      if (postsToday.length > 0) {
        log("📋 פוסטים שנבחרו להיום:");
        postsToday.forEach((post, index) => {
          log(`  ${index + 1}. ${post.filename || post.name}`);
          log(`     📊 סטטוס: ${post.status}`);
          log(`     🕒 זמן פרסום: ${post.publish_time || 'לא מוגדר'}`);
          log(`      סוג תזמון: ${post.schedule_type || 'לא מוגדר'}`);
          if (post.days_of_week) log(`     📅 ימי שבוע: ${post.days_of_week}`);
          if (post.one_time_date) log(`     📅 תאריך חד-פעמי: ${post.one_time_date}`);
          if (post.monthly_date) log(`     📅 תאריך חודשי: ${post.monthly_date}`);
          log("");
        });
        
        // בדיקת זמן פרסום עבור הפוסט הנבחר
        const selectedPost = postsToday[0];
        if (selectedPost.publish_time) {
          const shouldPublish = shouldPublishNow(selectedPost.publish_time);
          if (!shouldPublish) {
            log("⏰ החלטה: הפוסט לא יפורסם כי זמן הפרסום לא בטווח המותר - השרת יכבה");
          } else {
            log("✅ החלטה: הפוסט יפורסם כי זמן הפרסום בטווח המותר");
          }
        } else {
          log("📋 אין זמן פרסום מוגדר - הפוסט יפורסם בכל מקרה");
        }
      } else {
        log("📭 אין פוסטים נבחרים להיום - השרת יכבה");
      }
      
      log("✅ בדיקת לוגיקה הושלמה");
      return;
    }
